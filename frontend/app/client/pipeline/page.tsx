...
import { useClientLeads, useUpdateLeadStageMutation } from "@/lib/hooks/useClient";
...
import { Kanban, AlertTriangle, Phone, Mail, Calendar, Send } from "lucide-react";
import { useToast } from "@/lib/context/ToastContext";

export default function ClientPipelinePage() {
  const { toast } = useToast();
  const { data: leads = [], isLoading, isError } = useClientLeads();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[13px] text-[#6B7280]">
            Drag lead cards across stages to update status and track pipeline velocity
          </p>
        </div>
      </div>
      ...
      {/* Closed Lost Reason Modal (Required per DESIGN_SYSTEM.md & spec) */}
      <Modal
        ...
        description="Please document why this lead was marked as Closed Lost for analytics auditing."
      >
